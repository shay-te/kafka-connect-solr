...
public class SolrSinkConfig extends AbstractConfig {
    ...
    public static final String KAFKA_OFFSET_VERSION_FIELD_CONFIG = "kafka.offset.version.field";
    public static final String SOURCE_FIELD_CONFIG = "source.field";
    public static final String SOURCE_EXCLUDE_FIELDS_CONFIG = "source.exclude.fields";
    ...
    public static ConfigDef config() {
        ...
        def.define(KAFKA_OFFSET_VERSION_FIELD_CONFIG, Type.STRING, "", Importance.MEDIUM,
                "When set, every document is stamped with its Kafka offset in this field. Pair it with "
                        + "a Solr DocBasedVersionConstraints update processor (ignoreOldUpdates=true) on the "
                        + "same field so that with max.in.flight.requests > 1, an out-of-order (older-offset) "
                        + "write can never overwrite a newer version of the same id. Empty = disabled.",
                g, ++order, Width.MEDIUM, "Kafka offset version field");
        def.define(SOURCE_FIELD_CONFIG, Type.STRING, "", Importance.MEDIUM,
                "When set, the record value is stored verbatim as a JSON string in this field (a "
                        + "stored-only '_source'). Solr flattens nested arrays for indexing, so queries "
                        + "that need the original document (e.g. to return nested custom_fields) read this "
                        + "field back and parse it. Empty = disabled.",
                g, ++order, Width.MEDIUM, "Source (raw JSON) field");
        def.define(SOURCE_EXCLUDE_FIELDS_CONFIG, Type.LIST, java.util.Collections.emptyList(),
                Importance.LOW,
                "Comma-separated field-name PREFIXES to omit from the source.field JSON. Index-only "
                        + "denormalized fields (e.g. custom_field_, funnel_lead_) are excluded so the "
                        + "stored document stays the clean original shape. Empty = keep everything.",
                g, ++order, Width.MEDIUM, "Source exclude prefixes");
    ...
    public String kafkaOffsetVersionField() { return getString(KAFKA_OFFSET_VERSION_FIELD_CONFIG); }

    public String sourceField() { return getString(SOURCE_FIELD_CONFIG); }

    public java.util.List<String> sourceExcludeFields() { return getList(SOURCE_EXCLUDE_FIELDS_CONFIG); }
